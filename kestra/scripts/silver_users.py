...
def main():
    ...
    # Batch of Truth query: Matching the actual Bronze schema found in the logs
    query = f"""
        WITH deduped AS (
            SELECT 
                TRY_CAST(user_id AS BIGINT) AS user_id,
                TRY_CAST(current_age AS INTEGER) AS age,
                TRY_CAST(yearly_income AS DOUBLE) AS income,
                TRY_CAST(source_partner AS VARCHAR) AS partner,
                -- Since ingest_ts isn't in the logs' candidate list, 
                -- we'll use a placeholder or check your extractor.py
                NOW() AS ingest_ts, 
    ...
    table = catalog.load_table("silver.users")

    # Use table.upsert() to merge into the silver.users Iceberg table
